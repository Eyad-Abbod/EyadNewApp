...
class CommentsAllow extends StatefulWidget {
  const CommentsAllow({Key? key}) : super(key: key);

  @override
  State<CommentsAllow> createState() => _CommentsAllowState();
}

class _CommentsAllowState extends State<CommentsAllow> {
    ...
    if (_hasNextPage == true &&
      ...
      try {
        final res = await http
            .get(Uri.parse("$linkGetAllComments?page=$_page&state=1"));
  ...
  void _firstLoad() async {
    ...
    try {
      final res =
          await http.get(Uri.parse("$linkGetAllComments?page=$_page&state=1"));
      if (mounted) {
        setState(() {
