...
class DeletedNewsOperations extends StatefulWidget {
  final int newsType;
  const DeletedNewsOperations({Key? key, required this.newsType})
      : super(key: key);

  @override
  State<DeletedNewsOperations> createState() => _DeletedNewsOperationsState();
}

class _DeletedNewsOperationsState extends State<DeletedNewsOperations> {
    ...
    if (_hasNextPage == true &&
      ...
      try {
        final res = await http.get(Uri.parse(
            "$linkNewsOperations?page=$_page&newsState=3&newsType=${widget.newsType}"));
                    ...
                    if (response['status'] == 'success') {
                      if (mounted) {
                        AwesomeDialog(
                          ...
                          // dialogColor: AppTheme.appTheme.primaryColor,
                          title: 'نجاح',
                          desc: 'تمت عملية الاستعادة بنجاح',
                          btnOkOnPress: () {
                            // Get.back();
